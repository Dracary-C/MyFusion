...
```bash
cd ~/Experiment/All-in-One/TPGDiff/tpgd/src
...
```bash
cd ~/Experiment/All-in-One/TPGDiff/universal-restoration/config/tpgd-sde
多卡：
python -m torch.distributed.launch --nproc_per_node=2 --master_port=4321 train.py -opt=options/train.yml --launcher pytorch 2>&1 | tee ./run/run_$(date +%F_%H-%M-%S).log
单卡：
python train.py -opt=options/train.yml --launcher none 2>&1 | tee ./run/run_$(date +%F_%H-%M-%S).log
```
